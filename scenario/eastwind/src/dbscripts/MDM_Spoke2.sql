...
USE master
GO
if exists (select * from sysdatabases where name='EastwindMdmSpoke2DB')
		drop database EastwindMdmSpoke2DB
go
...
EXECUTE (N'CREATE DATABASE EastwindMdmSpoke2DB
  ON PRIMARY (NAME = N''EastwindMdmSpoke2DB'', FILENAME = N''' + @device_directory + N'EastwindMdmSpoke2DB.mdf'')
  LOG ON (NAME = N''EastwindMdmSpoke2DB_log'',  FILENAME = N''' + @device_directory + N'EastwindMdmSpoke2DB.ldf'')')
go

exec sp_dboption 'EastwindMdmSpoke2DB','trunc. log on chkpt.','true'
exec sp_dboption 'EastwindMdmSpoke2DB','select into/bulkcopy','true'
GO
