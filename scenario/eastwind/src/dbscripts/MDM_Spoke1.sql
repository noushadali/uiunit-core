...
USE master
GO
if exists (select * from sysdatabases where name='EastwindMdmSpoke1DB')
		drop database EastwindMdmSpoke1DB
go
...
EXECUTE (N'CREATE DATABASE EastwindMdmSpoke1DB
  ON PRIMARY (NAME = N''EastwindMdmSpoke1DB'', FILENAME = N''' + @device_directory + N'EastwindMdmSpoke1DB.mdf'')
  LOG ON (NAME = N''EastwindMdmSpoke1DB_log'',  FILENAME = N''' + @device_directory + N'EastwindMdmSpoke1DB.ldf'')')
go

exec sp_dboption 'EastwindMdmSpoke1DB','trunc. log on chkpt.','true'
exec sp_dboption 'EastwindMdmSpoke1DB','select into/bulkcopy','true'
GO
